...
)
{
	...
	iwp->length = 64;
	res = dev_wlc_bufvar_get(dev, "wpaie", extra, iwp->length);
	if (res)
		WL_ERROR(("%s : dev_wlc_bufvar_get error=%d\n", 
				  __FUNCTION__, res));
	return res;
}
...
static int get_softap_auto_channel(struct net_device *dev, struct ap_profile *ap, int ch)
{
	...
	int res = 0;
	/* restrict channel to 1 - 7 */
	uint32 req_buf[8] = {7, 1, 2, 3, 4, 5, 6, 7};
	...
	auto_channel_retry:
			...
			ret = dev_wlc_ioctl(dev, WLC_START_CHANNEL_SEL, &req_buf, sizeof(req_buf));
			if (ret < 0) {
	...
	get_channel_retry:
			...
			/* we have to use channel 1, 
			  * but why this code required?
			if ((chosen == 1) && (!rescan++))
...
exit:
	if (g_scan)
		kfree(g_scan);
	if (iscan && iscan->iscan_ex_params_p)
