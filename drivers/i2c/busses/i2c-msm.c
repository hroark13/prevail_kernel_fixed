...
msm_i2c_recover_bus_busy(struct msm_i2c_dev *dev, struct i2c_adapter *adap)
{
	...
	//cyj_test dev_err(dev->dev, "Bus still busy, status %x, intf %x\n",
	//cyj_test 	 status, readl(dev->base + I2C_INTERFACE_SELECT));
	enable_irq(dev->irq);
	return -EBUSY;
}
	...
	while (rem) {
		...
		if (check_busy) {
			...
			if (ret)
				...
				if (ret) {
					//cyj_test dev_err(dev->dev,
					//cyj_test 	"Error waiting for notbusy\n");
					goto out_err;
				}
			check_busy = 0;
		}
...
msm_i2c_probe(struct platform_device *pdev)
{
	...
	printk(KERN_INFO "msm_i2c: using pins %u (SDA) and %u (SCL)\n", dev->pdata->pri_dat, dev->pdata->pri_clk);
	clk_disable(dev->clk);
