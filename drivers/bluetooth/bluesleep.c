...
static void bluesleep_sleep_work(struct work_struct *work)
{
    if (bluesleep_can_sleep()) {
        ...
        if (test_bit(BT_ASLEEP, &flags)) {
            BT_INFO("bluesleep_sleep_work-already asleep");
            return;
        }

        if (msm_hs_tx_empty(bsi->uport)) {
            BT_INFO("bluesleep_sleep_work-going to sleep...");
...
{
    BT_INFO("bluesleep_hostwake_task-hostwake -> %u", gpio_get_value(bsi->host_wake));
...
static void bluesleep_tx_timer_expire(unsigned long data)
{
    ...
    BT_DBG("00 %x = %x", MSM_GPIO_OUT_1, readl(MSM_GPIO_OUT_1));
    BT_DBG("00 %x = %x", MSM_GPIO_OE_1, readl(MSM_GPIO_OE_1));
    BT_DBG("00 %x = %x", MSM_GPIO_IN_1, readl(MSM_GPIO_IN_1));
    BT_DBG("00 %x = %x", MSM_GPIO_INT_STATUS_1, readl(MSM_GPIO_INT_STATUS_1));
    // BT_DBG("-- %x = %x", 0xFA009470, readl(0xFA009470));
...
static int __init bluesleep_init(void)
{
    ...
    /*bgkim ef20 removed bluesleep init logs as per kernel team's request. printk(KERN_ERR "######################## bluesleep_init ########################");
    print_modules();
    dump_stack();
    printk(KERN_ERR "##############################################################");*/
