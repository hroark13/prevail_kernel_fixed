...
void get_usb_serial(char *usb_serial_number)
{
	...
	// change last bit of serial number to force a new driver installation
	unique_serial_number = unique_serial_number^0x1;
#endif

#if defined(CONFIG_MACH_RANT3)
	sprintf(temp_serial_number,"M580%08x",unique_serial_number);
#elif defined(CONFIG_MACH_REALITY2)
	sprintf(temp_serial_number,"I120%08x",unique_serial_number);
#elif defined(CONFIG_MACH_VINO)
	sprintf(temp_serial_number,"M820%08x",unique_serial_number);
#elif defined(CONFIG_MACH_GIOS)
	sprintf(temp_serial_number,"M828C%08x",unique_serial_number);	
#elif defined(CONFIG_MACH_CHIEF)
	sprintf(temp_serial_number,"D600%08x",unique_serial_number);
#elif defined(CONFIG_MACH_ROOKIE)
	sprintf(temp_serial_number,"R720%08x",unique_serial_number);
#elif defined(CONFIG_MACH_ESCAPE)
	sprintf(temp_serial_number,"R730%08x",unique_serial_number);
#elif defined(CONFIG_MACH_GIO)
	sprintf(temp_serial_number,"R680%08x",unique_serial_number);
#else
	sprintf(temp_serial_number,"S500%08x",unique_serial_number);
#endif
