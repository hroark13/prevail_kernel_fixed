...
rndis_setup(struct usb_function *f, const struct usb_ctrlrequest *ctrl)
{
	...
	case ((USB_DIR_OUT | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		//if (w_length > req->length || w_value || w_index != rndis->ctrl_id) // phill-it : for rndis issue (temporary) 2011.03.07
	...
	case ((USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE) << 8)
		...
		//if (w_value || w_index != rndis->ctrl_id) //  phill-it : for rndis issue (temporary) 2011.03.07
		if (w_value) 
			goto invalid;
		else {
			u8 *buf;
