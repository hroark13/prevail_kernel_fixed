...
#if TSP_TEST_MODE
static int tsp_mode = 0; 
#define TESTMODE_RAWCOUNTS 1
#define TESTMODE_IDAC 2
#define NORMAL_OPERATION_MODE 3
#endif
...
static ssize_t tkey_rawcounter_show_mcs7000(struct device *dev, struct device_attribute *attr, char *buf);
...
static ssize_t tsp_rawcounts_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t tsp_rawcounts_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
static ssize_t tsp_hst_mode_show(struct device *dev, struct device_attribute *attr, char *buf);
static ssize_t tsp_hst_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size);
#endif
static DEVICE_ATTR(tkey_rawcounter, S_IRUGO, tkey_rawcounter_show_mcs7000, NULL) ;
static DEVICE_ATTR(firmware	, S_IRUGO |S_IWUSR |S_IWGRP, firmware_show, firmware_store);
static DEVICE_ATTR(registers	, S_IRUGO | S_IWUSR , firmware_ret_show, firmware_ret_store);
#if TSP_TEST_MODE
static DEVICE_ATTR(tsp_name, S_IRUGO , tsp_name_show, NULL) ;
static DEVICE_ATTR(tsp_channel, S_IRUGO , tsp_channel_show, NULL) ;
static DEVICE_ATTR(tsp_rawcounts, S_IRUGO |S_IWUSR |S_IWGRP, tsp_rawcounts_show, tsp_rawcounts_store);
static DEVICE_ATTR(tsp_hst_mode, S_IRUGO |S_IWUSR |S_IWGRP, tsp_hst_mode_show, tsp_hst_mode_store); 
#endif
/* sys fs */
...
static void process_key_event(uint8_t tsk_msg)
{
	...
	for(i = 0; i < MAX_KEYS; i++)
	{
		...
		{
			// press event
			printk(KERN_DEBUG "[Touch Key] press \n");						
		...
		{
			// release event
			printk(KERN_DEBUG "[Touch Key] release \n");			
...
void TSP_forced_release_forkey(void)
{
	...
	for ( i= 0; i<MAX_USING_FINGER_NUM; i++ )
	{
		...
		printk(KERN_INFO "[TSP] force release\n");
		temp_value++;
	}
...
static void synaptics_ts_work_func(struct work_struct *work)
{
	...
	if (ret <= 0) {
		printk(KERN_ERR "[TSP] i2c failed : ret=%d, ln=%d\n",ret, __LINE__);
		goto work_func_out;
	}
	...
	for ( i= 0; i<MAX_USING_FINGER_NUM; i++ )
	{
		...
		//			if(fingerInfo[1].id ==0)
				...
				else if(fingerInfo[2].id != 0) // check x or y jump with same finger id
				{
					...
					if(ABS(fingerInfo[2].x,fingerInfo[0].x)>250)
					{
						input_report_abs(ts->input_dev, ABS_MT_POSITION_X, fingerInfo[2].x);	
						input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, fingerInfo[2].y);
						input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
					...
					else if(ABS(fingerInfo[2].y,fingerInfo[0].y)>250)
					{
						input_report_abs(ts->input_dev, ABS_MT_POSITION_X, fingerInfo[2].x);	
						input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, fingerInfo[2].y);
						input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
...
//	printk("\n");
	printk(KERN_DEBUG "[TSP] finger1:%d,%x	finger2:%d,%x\n", fingerInfo[0].status,fingerInfo[0].z ,fingerInfo[1].status ,fingerInfo[1].z);
	input_sync(ts->input_dev);
...
int tsp_i2c_read(u8 reg, unsigned char *rbuf, int buf_size)
{
	...
	if( ret < 0 )
		{
		printk(KERN_ERR "[TSP] Error code : %d\n", __LINE__ );
...
static void check_ic_work_func(struct work_struct *work)
{
	...
	if(check_ic_counter == 0)
	{
		...
		{
			printk(KERN_INFO "[TSP] %s tsp_reset counter = %x, prev = %x\n", __func__, wdog_val[0], (uint8_t)prev_wdog_val);
			tsp_reset();
			prev_wdog_val = -1;
		}
		else
		...
		struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (device_create_file(ts_dev, &dev_attr_tkey_rawcounter) < 0)
	...
	if (device_create_file(ts_dev, &dev_attr_tsp_rawcounts) < 0)    
		pr_err("Failed to create device file(%s)!\n", dev_attr_tsp_rawcounts.attr.name); 	
	if (device_create_file(ts_dev, &dev_attr_tsp_hst_mode) < 0)    
	    pr_err("Failed to create device file(%s)!\n", dev_attr_tsp_hst_mode.attr.name);                     
#endif
	...
	printk(KERN_INFO "[TSP] %s: %d, buf[1]=%x, buf[2]=%x\n", __func__, __LINE__, buf[1], buf[2]);
	...
	if (ret <= 0) {
		...
		if (ret <= 0) 
		{
			printk(KERN_ERR "[TSP] %s, ln:%d, Failed to register TSP!!!\n\tcheck the i2c line!!!, ret=%d\n", __func__,__LINE__, ret);
	...
	//if( buf[1]==0 || buf[2]==0 )  
	//{
	//	printk(KERN_ERR "[TSP] emergency firmware UPDATE\n");	
	//	firm_update();
	//}	
...
//			continue;
		...
		{
			printk(KERN_DEBUG "[TSP] %s:%d, ver SW=%x\n", __func__,__LINE__, buf[0] );
			enable_irq(client->irq);
			break;
		}

		if(retry_count > 5){
			printk(KERN_DEBUG "[TSP] %s: %d, retry_count=%d\n", __func__, __LINE__, retry_count);
			break;
		}
...
int set_tsp_for_prox_enable(int state)
{
	...
	if(ret == 0)
	{
		printk(KERN_ERR "[TSP] i2c failed\n");
	}
...
static ssize_t tkey_rawcounter_show_mcs7000(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint8_t tkey_rawcounter[2] = {0};
	uint8_t i2c_addr = 0x0E;
	int ret;

	ret = tsp_i2c_read( i2c_addr, tkey_rawcounter, sizeof(tkey_rawcounter));
	if (ret <= 0) {
		printk(KERN_ERR "i2c_transfer failed\n");
		ret = tsp_i2c_read( i2c_addr, tkey_rawcounter, sizeof(tkey_rawcounter));
		...
		if (ret <= 0) 
		{
			printk("[TSP] %s rawcounter read fail!!!!\n", __func__);
			goto error_return;
		}
	}

    printk(KERN_DEBUG "tkey_rawcounter : menu=%d back=%d \n", tkey_rawcounter[0], tkey_rawcounter[1]) ;

    return sprintf(buf, "%d %d", tkey_rawcounter[0], tkey_rawcounter[1]) ;
...
static ssize_t firmware_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if ( buf_tmp[0] > 0x9 )
		sprintf(buf, "Phone ver:%x HW:%x SW:%x\n", phone_ver, buf_tmp[0], buf_tmp[1]  );
	else
		sprintf(buf, "Phone ver:%x0 HW:%x SW:%x\n", phone_ver, buf_tmp[0],buf_tmp[1] );
...
{	
	...
	sprintf(buf, "[TOUCH] Cypress Tsp Info.\n");
	sprintf(buf, "%sHW : %x\n", buf,buf_tmp[0]);
	sprintf(buf, "%sSW : %x\n", buf,buf_tmp[1]);	
...
static ssize_t tsp_name_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "CYPRESS,TMA340\n");
}

static ssize_t tsp_channel_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "9,13\n");
}

static uint8_t set_Raw_counts_position = 1;

static ssize_t tsp_rawcounts_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    uint16_t rawcounts[9];
    read_dbg_data(tsp_mode, set_Raw_counts_position-1,&rawcounts);
    
	return sprintf(buf, "%d, %d, %d, %d, %d, %d, %d, %d, %d\n", 
	rawcounts[0],rawcounts[1],rawcounts[2],rawcounts[3],
	rawcounts[4],rawcounts[5],rawcounts[6],rawcounts[7],
	rawcounts[8]);
}
...
{	
    ...
    set_Raw_counts_position = (uint8_t)position;
...
static ssize_t tsp_hst_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%d", tsp_mode );
}

static ssize_t tsp_hst_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret=0;
	uint8_t i2c_addr_for_HSTMODE = 0x00;
	uint8_t i2c_addr_for_TTMODE = 0x01;    
	uint8_t wdog_val[1];

	if(*buf == '1') // TESTMODE_RAWCOUNTS
	{
		disable_irq(ts_global->client->irq);
		...
		wdog_val[0] = 0x40;
		ret = tsp_i2c_write( i2c_addr_for_HSTMODE, wdog_val, sizeof(wdog_val));
        tsp_mode = TESTMODE_RAWCOUNTS;
	}
	else if(*buf == '2') // TESTMODE_IDAC
	{
		disable_irq(ts_global->client->irq);
		ret = cancel_work_sync(&ts_global->work_timer);
		hrtimer_cancel(&ts_global->timer);
		wdog_val[0] = 0x60;
		ret = tsp_i2c_write( i2c_addr_for_HSTMODE, wdog_val, sizeof(wdog_val));
        tsp_mode = TESTMODE_IDAC;        
	}	
	else if(*buf == '3') // NORMAL_OPERATION_MODE
	{
		...
		wdog_val[0] = 0x50;
		ret = tsp_i2c_write( i2c_addr_for_HSTMODE, wdog_val, sizeof(wdog_val));
        tsp_mode = NORMAL_OPERATION_MODE; 
		msleep( 200 ); 
	}

	msleep( 200 );        

    printk("[TSP] %s tsp_mode(%d) = %d\n", __func__, tsp_mode, (wdog_val[0]));                   
}

void read_dbg_data(uint8_t dbg_mode , uint8_t node, uint16_t * dbg_data)
{
    uint8_t i2c_addr_for_rawcounts = 0x07;
    uint8_t wdog_val[1];
	uint8_t raw_val[9];
	uint8_t iDAC_val[18];
    int ret=0;    
    ...
    switch(dbg_mode)
    {
        case TESTMODE_RAWCOUNTS:  
            ret = tsp_i2c_read( i2c_addr_for_rawcounts+(node*9), raw_val, sizeof(raw_val));
            printk("[TSP] %s node(%d)= %d\n", __func__, node, (raw_val[0]));   

            if(ret <= 0)
            {
                // ERROR
                ret = tsp_i2c_read( i2c_addr_for_rawcounts+(node*9), raw_val, sizeof(raw_val));
                printk("[TSP] %s RAW node(%d) (i2c error-retry) = %d\n", __func__, node, (raw_val[0]));
            ...
            else
            {
                ...
                {
                    ret = tsp_i2c_read( i2c_addr_for_rawcounts+(node*9), raw_val, sizeof(raw_val));                    
                    printk("[TSP] %s RAW node(%d) (return 0-retry) = %d\n", __func__, node, (raw_val[0]));                                    
                    *dbg_data= raw_val[0];                                    
                }
                else {
					int i;
					for(i=0; i<9;i++){
					dbg_data[i] = raw_val[i]; 
					//printk("[TSP] %s raw_val= %d\n", __func__,raw_val[i]);
						}
                	}
            }
            break;
        case TESTMODE_IDAC:
            ret = tsp_i2c_read( i2c_addr_for_rawcounts, iDAC_val, sizeof(iDAC_val));
            if(ret <= 0)
            {
                // ERROR            
                ret = tsp_i2c_read( i2c_addr_for_rawcounts, iDAC_val, sizeof(iDAC_val));            
                printk("[TSP] %s DAC node(%d) (retry) = %d\n", __func__, node, (iDAC_val[0]));                
                *dbg_data= iDAC_val[0];                                                                      
            }
                else {
					int i;
					for(i=0; i<18;i++)
					dbg_data[i]= iDAC_val[i]; 
					//printk("[TSP] %s raw_val= %d\n", __func__,raw_val[i]);
