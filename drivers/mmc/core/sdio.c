			      ...
			      struct mmc_card *oldcard, int powered_resume)
{
	 ...
	 */
#if !defined(CONFIG_BCM4329) // match mmc clk and bcm4329 clk because of hotstpot lockup issue
...
{
	int i, err = 0;
#if !defined(CONFIG_BCM4329) //SecFeature.Quattro
	printk("[mjsmile]%s\n",__func__);
	for (i = 0; i < host->card->sdio_funcs; i++) {
