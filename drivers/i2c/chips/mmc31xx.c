...
#define USE_MMC3280 1
#else
extern unsigned char hw_version;

#if defined(CONFIG_MACH_VINO) || defined(CONFIG_MACH_GIOS)
#define USE_MMC3280 ( (hw_version >= 5 && hw_version < 90) ? 1 : 0) 
#elif defined(CONFIG_MACH_RANT3)
#define USE_MMC3280 ( hw_version >= 3 ? 1 : 0)
#else
#define USE_MMC3280 ( hw_version >= 1 ? 1 : 0)
#endif
#endif
	...
	unsigned int cmd, unsigned long arg)
{
	...
	case MMC31XX_IOC_TM:
		...
		/* [HSS] Magnetic sensor change : MMC3140 -> MMC3280 (since REV0.1) */
		if ( USE_MMC3280 ) 
		{	
			data[0] = MMC32XX_REG_CTRL;     
			data[1] = MMC31XX_CTRL_TM | MMC32XX_CTRL_NOBOOST;
...
{
	struct vreg *mmc31xx_vreg = vreg_get(0, LDO_LCD);
	unsigned char data[16] = {0};
	int res = 0;
...
void mmc31xx_early_suspend(struct early_suspend *);
void mmc3xx_late_resume(struct early_suspend *);

void mmc31xx_early_suspend(struct early_suspend *h)
{
	if ( !USE_MMC3280 ) 
	{
		struct vreg *mmc31xx_vreg = vreg_get(0, LDO_LCD);
...
void mmc31xx_late_resume(struct early_suspend *h)
{
	if ( !USE_MMC3280 ) 
	{
		struct vreg *mmc31xx_vreg = vreg_get(0, LDO_LCD);
...
static struct early_suspend mmc31xx_earlysuspend = {
	.suspend = mmc31xx_early_suspend,
	.resume = mmc31xx_late_resume,
};
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	register_early_suspend(&mmc31xx_earlysuspend);
#endif
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	unregister_early_suspend(&mmc31xx_earlysuspend);
#endif
