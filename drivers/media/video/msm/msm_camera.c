...
#endif//PCAM
#ifdef CONFIG_S5K4CA
extern void s5k4ca_rough_control(void __user *arg);
#endif
...
static int __msm_release(struct msm_sync *sync)
{
	...
	if (!sync->opencnt) {
		...
		if (sync->core_powered_on) {
			// Sensor relase first for i2c error
			/*sensor release */
			pr_info("%s, s_release\n", __func__);
			...
			struct msm_sensor_ctrl *))
{
	...
	pr_err("%s: setting camera node %d\n", __func__, camera_node);
	...
	pr_err("%s: setting camera type %d\n", __func__, sync->sctrl.s_camera_type);
    pr_err("%s: setting mount angle %d\n", __func__, sync->sctrl.s_mount_angle);
