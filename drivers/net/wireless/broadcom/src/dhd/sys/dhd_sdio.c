...
dhd_enable_oob_intr(struct dhd_bus *bus, bool enable)
{
	...
	if (dhdsdio_clkctl(bus, CLK_AVAIL, FALSE) < 0)
		printk (KERN_ERR "%s: clock control failed while enabling interrupt\n", __func__);
	if (enable == TRUE) {
...
dhd_bus_console_in(dhd_pub_t *dhdp, uchar *msg, uint msglen)
{
	 ...
	 */
	if (bus->clkstate == CLK_AVAIL &&
		((pkt = PKTGET(bus->dhd->osh, 4 + SDPCM_RESERVE, TRUE)) != NULL))
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *fw_path)
{
...
#ifdef VERIFY_FW
	readptr = MALLOC(bus->dhd->osh, MEMBLOCK + DHD_SDALIGN);
	if (!readptr)
		DHD_ERROR(("%s: Failed to allocate memory for read %d bytes\n", __FUNCTION__, MEMBLOCK));
	else
		safe_read = TRUE;
#endif //VERIFY_FW
	...
	while ((len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
		...
		if (safe_read)
		{
			memset (readptr, 0xaa, MEMBLOCK);
			bcmerror = dhdsdio_membytes(bus, FALSE, offset, readptr, len);
			if (bcmerror) {
				DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
			        	__FUNCTION__, bcmerror, MEMBLOCK, offset));
				goto err;
			}
			if (memcmp (readptr, memptr, len))
				printk ("Read compare failed\n");
			else
				printk ("Read comapre passed\n");
		}
...
#ifdef VERIFY_FW
	if (readptr)
		MFREE(bus->dhd->osh, readptr, MEMBLOCK + DHD_SDALIGN);
#endif //VERIFY_FW
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if (!embed)
	{
		printk (KERN_ERR "FIRMWARE PATH:%s\n", bus->fw_path);
		printk (KERN_ERR "NVRAM PATH:%s\n", bus->nv_path);
	}
	/* Keep arm in reset */
