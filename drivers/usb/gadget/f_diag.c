...
#ifdef DIAG_DEV_SEL
#define DIAG_DEV_UART3 0
#define DIAG_DEV_SMD 1

static int diag_dev_set(int dev)
{
	...
	if ( !(param_data = kzalloc(sizeof(struct samsung_parameter),GFP_KERNEL))) {
		printk("ERROR!! alloc failed for struct samsung_parameter(%d)\n", sizeof(struct samsung_parameter));
		return 0;
	}
	...
	if(dev == DIAG_DEV_UART3) {
		service = 0xf000; /* diag service */
		device = 0xf000; /* uart3 device */
		msm_proc_comm(PCOM_OEM_SANSUNG_RDM_ASSIGN_CMD, &service, &device);
		param_data->diag_dev_sel = DIAG_DEV_UART3;
		msm_write_param(param_data);
		printk("Diag device changed to UART3..\n");
	} else if(dev == DIAG_DEV_SMD) {
		service = 0xf000; /* diag service */
		device = 0xf001; /* usb diag device(it is actually a smd diag device) */
		msm_proc_comm(PCOM_OEM_SANSUNG_RDM_ASSIGN_CMD, &service, &device);
		param_data->diag_dev_sel = DIAG_DEV_SMD;
		msm_write_param(param_data);
		printk("Diag device changed to SMD..\n");
	} else {
		printk("Invalid diag device request!..\n");
		ret = -1;
	}
...
static int diag_dev_init(void)
{
	...
	diag_dev_set(param_data->diag_dev_sel);

	kfree(param_data);
}

static ssize_t diag_dev_switch_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct samsung_parameter *param_data;
	...
	switch(param_data->diag_dev_sel)
	{
		case 0:
			sprintf(buf, "Diag device : UART3");
			break;
		case 1:
			sprintf(buf, "Diag device : SMD");
			break;
		default:
			sprintf(buf, "Diag device : Invalid");
			break;
	}
...
static ssize_t diag_dev_switch_store(
		struct device *dev, struct device_attribute *attr,
		const char *buf, size_t size)
{	
	if(strncmp(buf, "UART3", 5) == 0 || strncmp(buf, "uart3", 5) == 0) {
		diag_dev_set(DIAG_DEV_UART3);
 	} else if(strncmp(buf, "SMD", 3) == 0 || strncmp(buf, "smd", 3) == 0) {
		diag_dev_set(DIAG_DEV_SMD);
	}

	return size;
}
static DEVICE_ATTR(diag_dev_sel, S_IRUGO |S_IWUGO | S_IRUSR | S_IWUSR, diag_dev_switch_show, diag_dev_switch_store);
#endif
...
static int __init usb_diag_init(void)
{
	...
	if (device_create_file(switch_dev, &dev_attr_diag_dev_sel) < 0)
		printk(KERN_ERR "[%s]: Failed to create device file(%s)!\n", __func__, dev_attr_diag_dev_sel.attr.name);
