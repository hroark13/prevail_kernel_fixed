					...
					enum led_brightness value)
{
	...
	if (!bl_lvl && value)
        ...
        if (!mfd->panel_power_on && bl_lvl != 0) { 
           msleep(100);
        }
#endif
...
extern void gpio_set_value(unsigned gpio, int on);
...
static int msm_fb_probe(struct platform_device *pdev)
{
   ...
   gpio_set_value(26, 0);
	...
	/* Mark for GetLog */
	frame_buf_mark.p_fb = fbram_phys - 0x13000000 + 0x03000000;
	Quattro_vendor1_id->flog_mark.p_fb   = fbram_phys - 0x13000000 + 0x03000000;
	...
	mfd->bl_level = mfd->panel_info.bl_max; // MBdkhan 20110214 0;
...
#ifdef CONFIG_MACH_RANT3_BST
	extern unsigned int customer_binary;
	if(customer_binary){
		if (!load_565rle_image_onfb( "SPH-M580BST_ROOTING.rle",0,0));
	}else{
		if (!load_565rle_image_onfb( "SPH-M580BST.rle",0,0));	/* Flip buffer */
	}
#else
	if (!load_565rle_image_onfb( "SPH-M580.rle",0,0));	/* Flip buffer */	
#endif	
#elif CONFIG_MACH_VINO
	if (!load_565rle_image_onfb( "SPH-M820.rle",0,0));	/* Flip buffer */
#elif CONFIG_MACH_GIOS
	if (!load_565rle_image_onfb( "SCH-M828C.rle",0,0)) ;	/* Flip buffer */	
#elif CONFIG_MACH_REALITY2
	//if (!load_565rle_image_onfb( "SCH-I120.rle",0,0));	/* Flip buffer */
#elif CONFIG_MACH_ROOKIE
	extern unsigned int customer_binary;
	if(customer_binary){
		if (!load_565rle_image_onfb( "SCH-R720_ROOTING.rle",0,0));	/* Flip buffer */
	}else{
		if (!load_565rle_image_onfb( "SCH-R720.rle",0,0));	/* Flip buffer */
	}
#elif CONFIG_MACH_ESCAPE
	extern unsigned int customer_binary;
	if(customer_binary){
		if (!load_565rle_image_onfb( "SCH-R730_ROOTING.rle",0,0));	/* Flip buffer */
	}else{
		if (!load_565rle_image_onfb( "SCH-R730.rle",0,0));	/* Flip buffer */
	}
#elif CONFIG_MACH_GIO
	extern unsigned int customer_binary;
	if(customer_binary){
		if (!load_565rle_image_onfb( "SCH-R680_ROOTING.rle",0,0));	/* Flip buffer */
	}else{
		if (!load_565rle_image_onfb( "SCH-R680.rle",0,0));	/* Flip buffer */
	}
#elif CONFIG_MACH_MARZO
	if (!load_565rle_image_onfb( "SWD-S300.rle",0,0));	/* Flip buffer */
#elif CONFIG_MACH_CHIEF
	if (!load_565rle_image_onfb( "SPH-D600.rle",0,0));	/* Flip buffer */	
#else
	if (!load_565rle_image_onfb( "SWD-S500.rle",0,0));	/* Flip buffer */
#endif
#endif
	ret = 0;
...
int get_fb_phys_info(unsigned long *start, unsigned long *len, int fb_num)
{
	...
	if (fb_num > MAX_FBI_LIST-1)// issue arrary index count overflow
		return -1;
