 ...
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See thefi
...
static void suspend(struct work_struct *work)
{
	int ret;
	int entry_event_num;
	unsigned int wakeup_status, pending_status;
	...
	if (debug_mask & DEBUG_EXIT_SUSPEND) {
		...
		ret = msm_proc_comm(PCOM_OEM_WAKEUP_STATUS, &pending_status, &wakeup_status);
		if(ret == 0)
		    pr_info("<<<==== wakeup_status : 0x%08x, pending_status : 0x%08x ====>>>\n", wakeup_status, pending_status);
#endif
	}
	...
	struct wake_lock *lock, long timeout, int has_timeout)
{
	...
	BUG_ON(type >= WAKE_LOCK_TYPE_COUNT);
	if(lock->flags == 0) {
		printk("%s : flag(0x%X), WAKE_LOCK_INITIALIZED(0x%X) ################\n",
				__func__, lock->flags, WAKE_LOCK_INITIALIZED);
		spin_unlock_irqrestore(&list_lock, irqflags);
		return;
	}
