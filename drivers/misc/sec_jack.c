...
/******** CHOICE OPTION by MODEL **********/
#define SUPPORT_PBA						/* for PBA factory test */
#define SUPPORT_MAX8899_PMIC_SENDEND	/* for detection sendend key by pmic without sendend gpio */	
/******************************************/
...
#ifdef SUPPORT_MAX8899_PMIC_SENDEND

struct sec_jack_info *g_hi;

static ssize_t h2w_print_name(struct switch_dev *sdev, char *buf)
{
	switch (switch_get_state(&switch_jack_detection)) {
...
#if 0		
	case H2W_SEC_HEADSET:
		switch_set_state(&switch_jack_detection, H2W_SEC_HEADSET);
		return 0;
#endif		
	case SEC_HEADSET_4POLE:
		switch_set_state(&switch_jack_detection, SEC_HEADSET_4POLE);
		return 0;
	default:
		break;
	}
...
{
	switch (switch_get_state(&switch_sendend)) {
	case 0:
...
{
	int state = *buf - 48;
	if(switch_get_state(&switch_jack_detection) == SEC_JACK_NO_DEVICE)
		return -EINVAL;
	pr_info(MODULE_NAME "button %s\n", state?"pressed":"released");
#if 0
	g_hi->button_state = state;
    switch_set_state(&g_hi->button_sdev, state);	
	hrtimer_start(&hi->btn_timer, hi->btn_debounce_time, HRTIMER_MODE_REL);
#endif
	//input_report_key(g_hi->input, KEY_MEDIA, state);
	input_report_key(g_hi->input, KEY_SEND, state);
	input_sync(g_hi->input);
	switch_set_state(&switch_sendend, state);
	g_hi->send_key_pressed = state;
	return 0;
}
#endif
...
static void set_send_key_state(struct sec_jack_info *hi, int state)
{
	...
	input_report_key(hi->input, KEY_SEND, state);
	input_sync(hi->input);
	switch_set_state(&switch_sendend, state);
...
#if !defined(SUPPORT_MAX8899_PMIC_SENDEND)
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	input_set_capability(hi->input, EV_KEY, KEY_SEND);
...
#ifdef SUPPORT_MAX8899_PMIC_SENDEND
	switch_jack_detection.print_name = h2w_print_name;
	switch_jack_detection.set_state = h2w_set_state;
	switch_sendend.print_name = button_print_name;
	switch_sendend.set_state = button_set_state;
#endif
...
#if !defined(SUPPORT_HSSD) && !defined(SUPPORT_MAX8899_PMIC_SENDEND)
...
MODULE_AUTHOR("inter.park@samsung.com");
