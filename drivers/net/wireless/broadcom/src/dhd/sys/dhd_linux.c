...
static int wifi_remove(struct platform_device *pdev)
{
	...
	wifi_set_carddetect(0);	//SecFeatue.Chief /* CardDetect (1->0) */
...
void * dhd_os_prealloc(int section, unsigned long size)
{
	...
	if (alloc_ptr) {
		...
		printk("dhd_os_prealloc");
		bzero(alloc_ptr, size);
		return alloc_ptr;
	}
	DHD_ERROR(("cannot alloc section %d\n", section));
	return NULL;	
#else
	u32 mem_start;	
	u32 mem_size;	
