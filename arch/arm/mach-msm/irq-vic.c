...
static uint8_t msm_irq_to_smsm[NR_IRQS] = { 0, } ;
static uint8_t msm_irq_to_smsm_without_uart3[NR_MSM_IRQS] = {
	[INT_MDDI_EXT] = 1,
	[INT_MDDI_PRI] = 2,
	[INT_MDDI_CLIENT] = 3,
	[INT_USB_OTG] = 4,
...
static uint8_t msm_irq_to_smsm_with_uart3[NR_MSM_IRQS] = {
	[INT_MDDI_EXT] = 1,
	[INT_MDDI_PRI] = 2,
	[INT_MDDI_CLIENT] = 3,
	[INT_USB_OTG] = 4,
...
#if defined(CONFIG_MSM_ARM9_USES_UART3)
	if(arm9_uses_uart3) {
		memcpy(msm_irq_to_smsm, msm_irq_to_smsm_without_uart3, ARRAY_SIZE(msm_irq_to_smsm_without_uart3));
	} else {
		memcpy(msm_irq_to_smsm, msm_irq_to_smsm_with_uart3, ARRAY_SIZE(msm_irq_to_smsm_with_uart3));
	}
#endif
