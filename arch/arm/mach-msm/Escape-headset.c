...
static void remove_headset(void)
{
	...
	gpio_set_value(GPIO_EARBIAS_EN, 0);     // earmic bias regulator off
...
static void insert_headset(void)
{
	unsigned long irq_flags;
	H2W_DBG("");
	gpio_set_value(GPIO_EARBIAS_EN, 1);     // earmic regulator on
...
//	printk("headset value2 = %d (%d retries)\n", value2, (10-retry_limit));
	...
	{ // 상태가 변할 때i i
		//gpio_set_value(GPIO_EARBIAS_EN, 1);	//popup_sw_en on
...
static int h2w_probe(struct platform_device *pdev)
{
	...
	hi->debounce_time = ktime_set(0, 1000000000);  /* 100 ms -> 200 ms -> 1 s */
...
/*****************************************************************/
//POPUP_SW_EN & EARMIC_BIAS_EN GPIO setting
/*****************************************************************/
	ret = gpio_request(GPIO_EARBIAS_EN, "h2w_earmic_bias");
	if (ret < 0)
		goto err_request_earbias_gpio;

	ret = gpio_direction_output(GPIO_EARBIAS_EN, 0);
	if (ret < 0)
		goto err_set_earbias_gpio;

    ret = gpio_request(GPIO_POPUP_SW_EN, "h2w_popup_sw") ;
    if (ret < 0)
		goto err_request_popup_sw_gpio;
	ret = gpio_direction_output(GPIO_POPUP_SW_EN, 0);
	if (ret < 0)
		goto err_set_popup_sw_gpio;
/*****************************************************************/
...
#endif
err_set_popup_sw_gpio:
    gpio_free(GPIO_POPUP_SW_EN) ;
err_request_popup_sw_gpio:
err_set_earbias_gpio:
	gpio_free(GPIO_EARBIAS_EN);
err_request_earbias_gpio:
...
MODULE_DESCRIPTION("SAMSUNG Headset driver for escape");
MODULE_LICENSE("GPL");
