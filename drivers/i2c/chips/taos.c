...
/* [HSS] This value depends on each model's H/W. It needs tunning */
	...
	#define PRX_THRSH_HI_PARAM		630 //Exchange GIO
	#define PRX_THRSH_LO_PARAM		500	//Exchange GIO
#else			
...
#elif defined(CONFIG_MACH_GIO) // Gio
#define PRX_PULSE_CNT_PARAM		0x07 //Exchange GIO
#else
#define PRX_PULSE_CNT_PARAM		0x08
#endif
#define PRX_GAIN_PARAM			0x20

#define TAOS_SENSOR_SDA 109
#define TAOS_SENSOR_SCL 110
...
static DEVICE_ATTR(proxsensor_file_state,0640, proxsensor_file_state_show, NULL);

/* for sensor test */
static ssize_t proxsensor_adc_show(struct device *dev, struct device_attribute *attr, char *buf)
{
...
static ssize_t proxsensor_adc_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	struct taos_data *taos = dev_get_drvdata(dev);
	bool new_value; 
	...
	mutex_lock(&taos->prox_lock);
	if(new_value) {
		hrtimer_start(&taos->timer, taos->prox_poll_delay, HRTIMER_MODE_REL);
		printk("[TAOS] hrtimer start for prox test! \n");
	}
	else if(!new_value) {
		hrtimer_cancel(&taos->timer);
		cancel_work_sync(&taos->work_test);
		printk("[TAOS] hrtimer for prox test stop! \n");
	}
	mutex_unlock(&taos->prox_lock);

	return size;
		
}

static DEVICE_ATTR(proxsensor_adc,0640, proxsensor_adc_show, proxsensor_adc_store);
...
/* for sensor test */
static void taos_work_func_test(struct work_struct *work)
{
	struct taos_data *taos = container_of(work, struct taos_data, work_test);
	printk("[TAOS] taos_work_func_test! %d \n" , PRX_PULSE_CNT_PARAM);
	proxsensor_get_avgvalue(taos);
}
...
static enum hrtimer_restart taos_prox_timer_func(struct hrtimer *timer)
{
	...
	queue_work(taos_test_wq, &taos->work_test);
	hrtimer_forward_now(&taos->timer, taos->prox_poll_delay);
...
/* End of code added by RSS 110130*/
	...
	if(adc_data > TAOS_PROX_MAX)
	{
		adc_data = TAOS_PROX_MAX;			
	}
	else if(adc_data < TAOS_PROX_MIN)
	{
		adc_data = TAOS_PROX_MIN;
	}
    ...
    if (prox_valid==0)
         proximity_value = 0;    //110217 hm83.cho  prox_valid==0 : saturation
...
/* Basic Software Operation for TMD2771X Module */
	...
	/* work queue settings for sensor test */
	taos_test_wq = create_singlethread_workqueue("taos_test_wq");
	if(!taos_test_wq)
		return -ENOMEM;
	INIT_WORK(&taos->work_test, taos_work_func_test);
	
	/* hrtimer setting for sensor test */
	hrtimer_init(&taos->timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	taos->prox_poll_delay= ns_to_ktime(2000 * NSEC_PER_MSEC);
	taos->timer.function = taos_prox_timer_func;

	mutex_init(&taos->prox_lock);
#endif
	...
	switch_cmd_dev = device_create(proxsensor_class, NULL, 0, "%s", "switch_cmd");	
	if (device_create_file(switch_cmd_dev, &dev_attr_proxsensor_file_state) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_proxsensor_file_state.attr.name);

	if (device_create_file(switch_cmd_dev, &dev_attr_proxsensor_adc) < 0)
		pr_err("Failed to create device file(%s)!\n", dev_attr_proxsensor_adc.attr.name);
...
exit_kfree:
	printk("taos_opt_probe is Failed!!\n");
	kfree(taos);
exit:
	printk("taos_opt_probe is Failed!!\n");
	return err;
...
int taos_chip_init(void)
{
	#ifdef TAOS_DEBUG
	gprintk("\n");
	#endif
	gpio_tlmm_config(GPIO_CFG(GPIO_SENSE_OUT, 0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);

	gpio_tlmm_config(GPIO_CFG(TAOS_SENSOR_SDA , 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(TAOS_SENSOR_SCL , 0, GPIO_CFG_OUTPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	vreg_proximity = vreg_get(0, "ldo9");

	vreg_set_level(vreg_proximity, OUT3000mV);
